...
require_once('GeppettoQueryRepository.php');
require_once('GeppettoValidation.php');

abstract class GeppettoFoundation
{
	// Generic cache
 ...
 ***************/
	...
	protected function _getTableSchema($table_name)
	{
		...
		if (isset(GeppettoFoundation::$_schema_cache[$table_name]))
		{
			return GeppettoFoundation::$_schema_cache[$table_name];
		}
		...
		GeppettoFoundation::$_schema_cache[$table_name] = $schema;
	...
	{
		// Primary key cache
		if (isset(GeppettoFoundation::$_primary_key_cache[$table_name]))
		{
			return GeppettoFoundation::$_primary_key_cache[$table_name];
		}
		...
		GeppettoFoundation::$_primary_key_cache[$table_name] = $rows[0]['primary_key'];
	...
	{
		// Reference Cache
		if (isset(GeppettoFoundation::$_references_cache[$table_name]))
		{
			return GeppettoFoundation::$_references_cache[$table_name];
		}
		...
		if (empty($rows))
		{
			...
			GeppettoFoundation::$_references_cache[$table_name] = array();
		...
		GeppettoFoundation::$_references_cache[$table_name] = $references;
	...
	private function _select($table_name, $columns, $primary_key_value)
	{
		...
		// Get the columns
		$parameter_columns = GeppettoValidation::validateTableColumns($table_name, $table_schema, $columns);
	...
	private function _insert($table_name, $keyed_array)
	{
		...
		list($parameter_columns, $parameter_values) = GeppettoValidation::validateTableInput($table_name, $table_schema, $table_primary_key, $keyed_array);
