...
require_once('GeppettoFoundation.php');

abstract class GeppettoObjectFoundation extends GeppettoFoundation
{
	/*
	 * Interfaces
	 */
	...
	{
		// IAS-specific
		if (class_exists($name . 'Dao') && is_subclass_of($name . 'Dao', '\Geppetto\GeppettoObject'))
		{
			// Create the class name
			$class_name = $name . 'Dao';
		}
		// Generic
		else if (class_exists($name) && is_subclass_of($name, '\Geppetto\GeppettoObject'))
		{
			// Create the class name
			$class_name = $name;
		}
